...
class CollageMaker():
    ...
    def __init__(
        ...
        if self._use_prompt:
            ...
            self._prompt_features = training.compute_image_features(
                self._prompts, self._clip_model, self._device)
    ...
    def load(self, path_checkpoint):
        ...
        state_dict = torch.load(
            path_checkpoint, map_location=self._device.type)
        ...
        for key in this_state_dict:
            ...
            if this_shape != shape:
                print(
                    f"state_dict[{key}] do not match: {this_shape} vs. {shape}")
...
class CollageTiler():
    ...
    def __init__(self,
        ...
        self._height = int(
            ((2 * self._tiles_high + 1) * self._tile_height) / 3.)
    ...
    def _print_info(self):
        ...
        print(
            f"Global size: {self._high_res_width}x{self._high_res_height} (WxH)")
    ...
    def loop(self):
        ...
        with open(res_filename, "w") as f:
            yaml.dump(res_training, f, default_flow_style=False,
                      allow_unicode=True)
    ...
    def _get_tile_background(self):
        ...
        if "above_right" in tile_idx:
            self._copy_overlap(tile_border_bg, "above_right",
                               tile_idx["above_right"])
        if "left" in tile_idx:
