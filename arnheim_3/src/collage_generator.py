...
class PopulationCollage(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.mask_transform = torch.nn.Parameter(torch.zeros(
            pop_size, self._num_patches, self._canvas_height, self._canvas_width), requires_grad=False).to(self.device)
    ...
    def _fetch_patch(self, idx_population, idx_patch, is_high_res):
        ...
        patch_location = torch.zeros(
            self._canvas_height, self._canvas_width).to(self.device)
        patch_location[w0:(w0 + width_j), h0:(h0 + height_j)] = 1.0
        return mapped_patch, patch_location

    def copy_and_mutate_s(self, parent, child):
        with torch.no_grad():
            # Copy the patches indices from the parent to the child.
            self.patch_indices[child] = copy.deepcopy(
                self.patch_indices[parent])
    ...
    def copy_from(self, other, idx_to, idx_from):
        ...
        with torch.no_grad():
            self.mask_transform[idx_to, ...] = other.mask_transform[idx_from, ...]
            self.mask = copy.deepcopy(other.mask)
            self.patch_indices[idx_to] = copy.deepcopy(
                other.patch_indices[idx_from])
    ...
    def forward(self, params=None):
        ...
        # self._pop_size, self._num_patches, 5, self._canvas_height, self._canvas_width
        # torch.Size([1, 100, 5, 224, 224])
    ...
    def forward_high_res(self, params=None):
        ...
        for u in range(num_w):
            for v in range(num_h):
                ...
                coloured_patches_uv = self.colour_transformer(
                    shifted_patches_uv)
                if self.config["render_method"] == "transparency":
