...
def compute_image_features(images, clip_model, device):
    ...
    for n in range(num_augs):  # number of augmentations or composition images
        for p in range(pop_size):
            ...
            if VISUALISE_BATCH_IMAGES and t % 500 == 0:
                ...
                if config["compositional_image"]:
                    print(
                        f"Loss {loss} for image region with prompt {prompts[n]}:")
                else:
                    print(
                        f"Loss {loss} for image augmentation with prompt {prompts[0]}:")
